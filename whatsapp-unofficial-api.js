...
class whatsappUnofficialApi {
     ...
     async services (save,headdless) {
         ...
         global.selectors = ["/html/body/div/div[1]/div/div[2]/div[1]/div/div[3]/label/input",
    ...
    async login(keep,page=global.page) {
        ...
        global.qr = new JSSoup(await page.evaluate(() => document.querySelector('*').outerHTML)).findAll('div', {class : '_2UwZ_'})[0]["attrs"]["data-ref"]
    ...
    async QrDetection(page=global.page,qr=global.qr) {
        await (async () => {
            while(await true) {
                ...
                try {
                    var newqr = new JSSoup(await page.evaluate(() => document.querySelector('*').outerHTML)).findAll('div', {class: '_2UwZ_'})[0]["attrs"]["data-ref"]
                } catch (error) {
    ...
    async getfriendslist(range,page=global.page) {
        ...
        for (let i = 0; i < range; i++) {
            try {
                let [element] = await page.$x('/html/body/div/div[1]/div[1]/div[3]/div/div[2]/div[1]/div/div/div[' + i + ']/div/div/div[2]/div[1]/div[1]/span/span');
